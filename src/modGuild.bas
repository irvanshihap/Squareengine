...
Public Const MAX_GUILD_MEMBERS As Long = 15
...
Public Sub MakeGuild(Founder_Index As Long, Name As String, Tag As String)
    ...
    If Not Options.Buy_Item = 0 Then
        ...
        ' Gold Req
        If itemamount = 0 Or itemamount < Options.Buy_Lvl Then
            PlayerMsg Founder_Index, "You need " & Options.Buy_Cost & " " & Item(Options.Buy_Item).Name & " to make a guild!", BrightRed
            Exit Sub
        End If
        
        'Take the item
        TakeInvItem Founder_Index, Options.Buy_Item, Options.Buy_Lvl
    End If
    
    
    GuildData(GuildSlot).Guild_Name = Name
    GuildData(GuildSlot).Guild_Tag = Tag
    GuildData(GuildSlot).Guild_Color = Red
